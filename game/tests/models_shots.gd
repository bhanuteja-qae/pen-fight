extends SceneTree
class_name ModelsShot
## Model-set skin mockup capture: swaps the NB2-generated REAL-PEN skins
## (game/assets/nb2/models/) onto the REAL scene and saves engine-side frames
## at true in-game scale, exactly like tests/nb2_shots.gd does for the
## identity set.
##
...
##   ../tools/godot-run.sh --path . --script res://tests/models_shots.gd
## Writes frames to OUT_DIR (absolute, outside the repo).

const OUT_DIR := "/home/ubuntu/game-asset-model-research/real-pens/frames"
...
## Swap one player's pen to a model design (pen + shadow, same scale).
func _apply(player: String, design: String) -> bool:
	...
	var pen_path := "res://assets/nb2/models/pen_%s.png" % design
	var sh_path := "res://assets/nb2/models/pen_%s_shadow.png" % design
	if not ResourceLoader.exists(pen_path) or not ResourceLoader.exists(sh_path):
		print("MODELS SHOT: missing %s / %s" % [pen_path, sh_path])
		return false
...
func _go() -> void:
	...
	if img == null:
		print("MODELS SHOT FAILED: viewport image null (baseline)")
		quit(1)
		return
	img.save_png("%s/00_defaults_sharpie_vs_bic.png" % OUT_DIR)

	# 1) each model pairing, plus a no-pen reference frame so a diff yields the
	...
	for idx in range(PAIRINGS.size()):
		...
		if shot == null:
			print("MODELS SHOT FAILED: viewport image null (%s/%s)" % [a, b])
			quit(1)
			return
		var path := "%s/%02d_models_%s_vs_%s.png" % [OUT_DIR, idx + 1, a, b]
		shot.save_png(path)
